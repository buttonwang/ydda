...
public class Note extends Model {
    ...
    public String getApproveLevelName() {
        ...
        if(approveLevel != null && !(approveLevel.equals(""))){
            if (approveLevel==0) {
                ...
                return "存档";
            }else if (approveLevel==7){
                return "科室审核未通过";
            }else if (approveLevel==8) {
                Role role=Role.findById((Long.parseLong("4")));
                if(role!=null){
    ...
    public static String toCheckJson(List<Note> notes) {
        return new JSONSerializer()
    	  ...
    	  .include("id","noteYear", "noteDate", "content",
                  "fileSrc", "certifyMan","approveLevel",
                  "approveLevelName","checkTitle","checkId","userId","deptId","realName","deptName",
                  "category","categoryName","approveComment","score","goods","createdManName","money")
    	  .exclude("*")
    	  .serialize(notes);
    }
    ...
    public static String toApproveJson(List<Note> notes) {
    	return new JSONSerializer()
    	  .transform(new DateTransformer("yyyy-MM-dd"), "noteDate")
    	  .include("id", "realName", "checkTitle", "noteDate", "content", "fileSrc", "approveLevel",
                  "approveLevelName","certifyMan","deptName","categoryName","approveComment","score",
                  "goods","createdManName","money")
    	  .exclude("*")
    	  .serialize(notes);
    }
